...
SIGN_MAX_OFFSET_M = 15.0        # a sign node sits on the road -> keep tight
EARTH_R = 6_371_000.0
...
def next_sign_ahead(pose: tuple[float, float], heading: float | None,
                    road: Road) -> tuple[float, str] | None:
    """(distance_m, kind) of the nearest stop/give-way sign ahead on this road.

    Same shape and fail-safe as next_curve_ahead: needs a heading, and the sign
    node must project onto the road. Heads-up only -- returns None when nothing is
    ahead, so a sign already behind us never shows.
    """
    if heading is None or not road.signs or len(road.points) < 2:
        return None
    ref = road.points[0]
    pts = [_to_xy(ref, p) for p in road.points]
    cum = [0.0]
    ...
    for sg in road.signs:
        s_sg, off = _project(pts, cum, _to_xy(ref, (sg.lat, sg.lon)))
        if off > SIGN_MAX_OFFSET_M:
            continue
        ahead = (s_sg - s_pose) if forward else (s_pose - s_sg)
        if ahead <= 0:
            continue
        if best is None or ahead < best[0]:
            best = (round(ahead, 1), sg.kind)
    return best
...
def _advisory(road: Road, distance_m: float, pose: tuple[float, float],
    ...
    sg = next_sign_ahead(pose, heading, road)
    return Advisory(
        valid=True,
        road_id=road.id,
        ...
        sign_distance_m=sg[0] if sg else None,
        sign_kind=sg[1] if sg else "",
    )
