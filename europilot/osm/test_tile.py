...
CAMERAS = (Path(__file__).resolve().parents[2]
           / "gateway" / "osm" / "test_data" / "cameras.osm").read_bytes()
...
class TestDecode:
    ...
    def test_cameras_decode_from_gateway_bytes(self):
        tiles = build_tiles_full(parse_full(CAMERAS))
        (lat, lon), records = next((k, v) for k, v in tiles.items()
                                   if any(r["id"] == 200 for r in v))
        roads = decode_tile(serialize_tile(lat, lon, records))
        mainline = next(r for r in roads if r.id == 200)
        assert {c.kind for c in mainline.cameras} == {"fixed", "section"}
        assert any(c.maxspeed == 100 for c in mainline.cameras)
        assert all(50.0 < c.lat < 54.0 for c in mainline.cameras)   # decoded to degrees

    def test_camera_less_road_decodes_empty(self):
        rec = {"id": 9, "roadClass": "primary", "maxspeed": 50, "oneway": True,
               "lanes": 2, "name": None, "coords": [(52.0, 5.0), (52.0, 5.001)]}
        assert decode_tile(serialize_tile(208, 20, [rec]))[0].cameras == ()
